...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        known_mines_count = sum (1 for n in neighbors if n in self.mines)
        new_count = count - known_mines_count

        if len(unknown_neighbors) > 0 and 0 <= new_count <= len(unknown_neighbors):
            new_sentence = Sentence(unknown_neighbors, new_count)
            self.knowledge.append(new_sentence)

        interference_made = True
        while interference_made:
            interference_made = False
            ...
            for i in range(len(self.knowledge)):
                for j in range(len(self.knowledge)):
                    if i != j and self.knowledge[i].cells.issubset(self.knowledge[j].cells):
